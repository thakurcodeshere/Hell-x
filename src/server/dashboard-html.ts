  ...
  <script>
    ...
    async function loadData() {
      try {
        ...
        // Render Evidence
        const evidBody = document.getElementById('evidence-table-body');
        if (evidenceRes.data) {
          evidBody.innerHTML = evidenceRes.data.map(e => \`
        ...
        // Render Memory
        const memContainer = document.getElementById('memory-cards-container');
        if (memoryRes.data) {
          memContainer.innerHTML = memoryRes.data.map(m => \`
            <div class="bg-slate-900 p-4 rounded-lg border border-slate-800 space-y-2 text-xs">
    ...
    async function launchMission() {
      ...
      badge.className = 'px-2 py-0.5 rounded bg-amber-500/20 text-amber-400 font-bold animate-pulse';

      term.innerHTML += \`<p class="text-cyan-400">[Mission] Initiating autonomous mission: "\${input}"...</p>\`;
      ...
      if (res.success) {
        ...
        badge.className = 'px-2 py-0.5 rounded bg-emerald-500/20 text-emerald-400 font-bold';
        term.innerHTML += \`<p class="text-emerald-400">✓ Mission \${res.data.missionId} completed successfully! All 6 Gates Passed. Release: \${res.data.releaseVersion}</p>\`;
        loadData();
      } else {
        badge.innerText = 'FAILED';
        badge.className = 'px-2 py-0.5 rounded bg-red-500/20 text-red-400 font-bold';
        term.innerHTML += \`<p class="text-red-400">❌ Mission failed: \${res.error}</p>\`;
      }
      term.scrollTop = term.scrollHeight;
    }
